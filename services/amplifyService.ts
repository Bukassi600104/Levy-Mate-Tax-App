...
export const createProfile = async (profile: Omit<TaxProfile, 'id'>): Promise<any> => {
  try {
    const result = await client.models.TaxProfile.create({
      ...
      // NTA 2025: New fields
      nhfOptIn: profile.nhfOptIn || false,
      nhisContribution: profile.nhisContribution || 0,
      employerSector: profile.employerSector || 'Private',
      rentPaid: profile.rentPaid,
      rentVerified: profile.rentVerified !== false, // Default to true
      lifeInsurance: profile.lifeInsurance,
      lifeInsuranceSpouse: profile.lifeInsuranceSpouse || 0,
...
/**
 * Fetch the current user's profile (FAST - no transactions)
 * Use this for initial load, then load transactions separately
 */
export const getProfileFast = async (): Promise<any> => {
  try {
    const result = await client.models.TaxProfile.list();
    const profile = result.data?.[0] || null;
    
    if (profile) {
      // Return profile with empty transactions - load them later
...
/**
 * Fetch the current user's profile (FULL - with transactions)
