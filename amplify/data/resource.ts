...
/**
 * Data Schema - LevyMate Tax App (NTA 2025 Updated)
...
const schema = a.schema({
  ...
  TaxProfile: a.model({
    ...
    // Statutory Deductions (monthly amounts)
    pensionContribution: a.float(), // monthly
    nhfContribution: a.float(), // monthly (legacy)
    
    // NTA 2025: New Statutory Fields
    nhfOptIn: a.boolean(), // NHF voluntary opt-in for private sector
    nhisContribution: a.float(), // NHIS monthly contribution
    employerSector: a.enum(['Public', 'Private']), // Affects mandatory NHF

    // Reliefs
    rentPaid: a.float(), // annual rent for Rent Relief
    rentVerified: a.boolean(), // Documentation verified
    lifeInsurance: a.float(), // annual (self)
    lifeInsuranceSpouse: a.float(), // NTA 2025: Spouse life insurance
    
    // NTA 2025: Capital Gains (Harmonized with PIT)
    capitalGains: a.float(), // Annual capital gains

    // Subscription & Usage
    tier: a.enum(['Free', 'Pro']),
    subscriptionPlan: a.enum(['Monthly', 'Yearly']),
    subscriptionExpiryDate: a.string(),
    aiQueriesToday: a.integer(), // Resets daily
