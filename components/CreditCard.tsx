  ...
  return (
    <div className={`relative w-full aspect-[1.586] rounded-2xl ${bgClass} p-6 text-white shadow-2xl overflow-hidden transition-transform duration-500 hover:scale-[1.02] group border-t border-white/10`}>
      ...
      <div className="relative z-10 flex flex-col justify-between h-full">
        ...
        {/* Bottom Row: Info */}
        <div className="flex justify-between items-end font-mono gap-2">
          <div className="min-w-0 flex-1">
            <div className="text-[8px] md:text-[9px] text-blue-200 uppercase mb-0.5 opacity-70">Taxpayer</div>
            <div className="text-xs md:text-sm lg:text-base font-bold tracking-wider uppercase truncate">
              {name || 'VALUED USER'}
            </div>
          </div>
          <div className="text-right flex-shrink-0">
            <div className="text-[8px] md:text-[9px] text-blue-200 uppercase mb-0.5 opacity-70">Status</div>
            <div className="text-[10px] md:text-xs font-bold bg-white/20 px-1.5 md:px-2 py-0.5 rounded backdrop-blur-md whitespace-nowrap">
                {label}
            </div>
          </div>
        </div>
