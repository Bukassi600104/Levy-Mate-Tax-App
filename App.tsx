...
import { createProfile, getProfile, getProfileFast, updateProfile, getTransactionsByProfile } from './services/amplifyService';
...
const App: React.FC = () => {
  ...
  const [viewState, setViewState] = useState<'landing' | 'features' | 'how-it-works' | 'auth' | 'loading' | 'feedback'>('loading');
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isAuthenticatedOnLanding, setIsAuthenticatedOnLanding] = useState(false);
  ...
  const [formData, setFormData] = useState({
      ...
      turnover: 0,
      rentPaid: 0,
      employerSector: 'Private' as 'Public' | 'Private', // NTA 2025
      state: 'Lagos',
      phone: '+234',
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        if (isAuth) {
          // User is signed in, load their profile fast (without transactions)
          const userProfile = await getProfileFast();
          if (userProfile) {
            setProfile(userProfile as TaxProfile);
            setViewState('auth');
            sessionStorage.setItem('levymate_auth_state', 'authenticated');
          } else {
          ...
          sessionStorage.setItem('levymate_auth_state', 'unauthenticated');
          setIsAuthenticatedOnLanding(false);
  ...
  useEffect(() => {
    const checkAuthOnLanding = async () => {
      if (viewState === 'landing' || viewState === 'features' || viewState === 'how-it-works') {
        try {
          const isAuth = await authIsAuthenticated();
          if (isAuth) {
            const user = await authGetCurrentUser();
            setIsAuthenticatedOnLanding(true);
            setAuthenticatedUserName(user?.email || user?.username || '');
          } else {
            setIsAuthenticatedOnLanding(false);
  ...
  // Navigate to dashboard from landing page (when already authenticated)
  const handleGoToDashboard = async () => {
    try {
      ...
      console.error('Failed to go to dashboard:', err);
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if profile already exists (in case user refreshed during onboarding)
      const existingProfile = await getProfileFast();
      if (existingProfile) {
        setProfile(existingProfile as TaxProfile);
        sessionStorage.setItem('levymate_auth_state', 'authenticated');
        setIsLoading(false);
        return;
      }
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Load user's profile fast (without transactions) for quick dashboard access
      const userProfile = await getProfileFast();
      if (userProfile) {
        setProfile(userProfile as TaxProfile);
        sessionStorage.setItem('levymate_auth_state', 'authenticated');
        // Cache profile ID for background transaction loading
        sessionStorage.setItem('levymate_profile_id', userProfile.id);
      } else {
        setAuthView('onboarding');
        setStep(1);
      }
      ...
      // Handle "already signed in" error - redirect to dashboard
      if (err.message?.includes('already a signed in user') || err.name === 'UserAlreadyAuthenticatedException') {
        try {
          ...
          console.error('Failed to load profile for existing session:', profileErr);
        }
      }
  ...
  const handleStart = async (plan?: 'Monthly' | 'Yearly') => {
    ...
    const newProfile: TaxProfile = {
      ...
      nhfContribution: 0,
      nhfOptIn: false, // NTA 2025: Default to not opted in for private sector
      nhisContribution: 0,
      employerSector: formData.employerSector, // NTA 2025: From onboarding
      rentPaid: formData.rentPaid,
      rentVerified: true, // Default to verified for user-entered data
      lifeInsurance: 0,
      lifeInsuranceSpouse: 0, // NTA 2025: Spouse insurance
      capitalGains: 0, // NTA 2025: Capital gains field
      transactions: [],
      tier: formData.tier,
  ...
  const handleLogout = async () => {
    try {
      await authSignOut();
      // Clear auth cache
      sessionStorage.removeItem('levymate_auth_state');
      setProfile(null);
      setFormData({
        name: '',
        email: '',
  ...
  if (profile) {
    return (
        <Suspense fallback={
            ...
            <Helmet>
                <title>Dashboard - LevyMate</title>
            </Helmet>
            <Dashboard 
                profile={profile} 
                onLogout={handleLogout} 
                onProfileUpdate={setProfile} 
                onViewFeedback={() => setViewState('feedback')}
            />
        </Suspense>
    );
  }
      ...
      return (
        <Suspense fallback={
            ...
            <LandingPage 
                ...
                onViewFeedback={() => setViewState('feedback')}
                isAuthenticated={isAuthenticatedOnLanding}
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
        <Helmet>
            <title>
                {authView === 'login' ? 'Login - LevyMate' :
                 authView === 'register' ? 'Create Account - LevyMate' :
                 authView === 'forgot-password' ? 'Reset Password - LevyMate' :
                 authView === 'reset-password' ? 'New Password - LevyMate' :
                 authView === 'confirm-email' ? 'Verify Email - LevyMate' :
                 authView === 'onboarding' ? 'Setup Profile - LevyMate' : 'LevyMate'}
            </title>
        </Helmet>
        <PolicyModal isOpen={policyModalOpen} onClose={() => setPolicyModalOpen(false)} type={policyType} />
        <CheckoutModal 
                ...
                <div className="w-full max-w-md bg-white p-8 lg:p-10 rounded-3xl shadow-xl shadow-slate-200/50 border border-white relative animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {authView === 'onboarding' && (
                        <>
                            ...
                            {step === 3 && (
                                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                    <h2 className="text-2xl font-bold text-gray-900">Location & Employment</h2>
                                    ...
                                    {/* NTA 2025: Employer Sector (for Individuals) */}
                                    {formData.entityType === EntityType.INDIVIDUAL && (
                                        <div>
                                            <label className="text-xs font-bold text-gray-500 uppercase block mb-2 flex items-center gap-1">
                                                <Briefcase size={12}/> Employer Sector (NTA 2025)
                                            </label>
                                            <div className="grid grid-cols-2 gap-3">
                                                <button
                                                    type="button"
                                                    onClick={() => setFormData({...formData, employerSector: 'Private'})}
                                                    className={`p-4 rounded-xl border-2 text-center transition-all ${
                                                        formData.employerSector === 'Private'
                                                            ? 'border-levy-blue bg-blue-50 text-levy-blue'
                                                            : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300'
                                                    }`}
                                                >
                                                    <div className="font-bold">Private Sector</div>
                                                    <div className="text-[10px] mt-1 opacity-70">NHF is voluntary</div>
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => setFormData({...formData, employerSector: 'Public'})}
                                                    className={`p-4 rounded-xl border-2 text-center transition-all ${
                                                        formData.employerSector === 'Public'
                                                            ? 'border-levy-blue bg-blue-50 text-levy-blue'
                                                            : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300'
                                                    }`}
                                                >
                                                    <div className="font-bold">Public Sector</div>
                                                    <div className="text-[10px] mt-1 opacity-70">NHF is mandatory</div>
                                                </button>
                                            </div>
                                            <p className="text-[11px] text-amber-700 mt-2 bg-amber-50 p-2 rounded-lg">
                                                NTA 2025: NHF contributions are now voluntary for private sector employees.
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    <div>
                                        ...
                                        />
                                        <p className="text-[11px] text-gray-500 mt-1.5">
                                            We will send you SMS reminders so you don't miss filing deadlines.
                                        </p>
                                    </div>
